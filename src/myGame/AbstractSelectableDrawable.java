...
/**
 * Convenience base abstract class for SelectableDrawable objects that can have
 * a transformation matrix applied before drawing. Created by Gustavo on
 * 2017-06-17.
 */
public abstract class AbstractSelectableDrawable implements SelectableDrawable {
  ...
  /**
   * Returns the current transformation matrix for this SelectableDrawable. This
   * matrix can be further manipulated to change the transformation.
   * 
   * @return
   */
  ...
  /**
   * Transforms mouse coordinates of a point to model coordinates considering
  ...
  public Point2D transformMousePointToModel(Point2D point) throws NoninvertibleTransformException {
    ...
    if (this.transform != null) {
      this.transform.inverseTransform(point, transformedPoint); // use inverse
                                                                // transformation!
    }
