...
/**
 * Convenience base abstract class for SelectableDrawable objects that can be
 * manipulated (translated and scaled). Created by Gustavo on 2017-06-18.
 */
public abstract class AbstractManipulableDrawable extends AbstractSelectableDrawable {
  ...
  /**
   * Paint this SelectableDrawable using the specified Graphics2D object. First,
   * this method will call paintItself() for the actual drawing. Next, if this
   * SelectableDrawable is selected, it will draw a bounding box;
   * 
   * @param g2
   */
  @Override
  public void paint(Graphics2D g2) {
    ...
    if (this.isSelected()) {
      ...
      Stroke dashed = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] { 2 }, 0); // create
                                                                                                                 // dashed
                                                                                                                 // stroke
      g2.setStroke(dashed);
      Shape boundingBox = this.transform == null ? this.getBoundingBox() : this.transform.createTransformedShape(this.getBoundingBox()); // transform
                                                                                                                                         // coordinates
  ...
  /**
   * Translates this SelectedDrawable by the specified amount (this is
   * cumulative to the current transformation.)
   * 
   * @param tx
   * @param ty
   */
  ...
  /**
   * Scales this SelectedDrawable by the specified amount (this is cumulative to
  ...
  /**
   * Test if the given point (px, py) is on top of one of the lines of the
   * bounding box.
   * 
   * @param px
   * @param py
  ...
  public BoundingBoxBorder isInBoundingBox(double px, double py) throws NoninvertibleTransformException {
    ...
    if (this.withinTolerance(transformedPoint.getX(), boundingBox.getMinX())) {
      if (transformedPoint.getY() > boundingBox.getMinY() && transformedPoint.getY() < boundingBox.getMaxY()) {
        return BoundingBoxBorder.WEST;
      }
    } else if (this.withinTolerance(transformedPoint.getX(), boundingBox.getMaxX())) {
      if (transformedPoint.getY() > boundingBox.getMinY() && transformedPoint.getY() < boundingBox.getMaxY()) {
        return BoundingBoxBorder.EAST;
      }
    } else if (this.withinTolerance(transformedPoint.getY(), boundingBox.getMinY())) {
      if (transformedPoint.getX() > boundingBox.getMinX() && transformedPoint.getX() < boundingBox.getMaxX()) {
        return BoundingBoxBorder.NORTH;
      }
    } else if (this.withinTolerance(transformedPoint.getY(), boundingBox.getMaxY())) {
      if (transformedPoint.getX() > boundingBox.getMinX() && transformedPoint.getX() < boundingBox.getMaxX()) {
   ...
   */
  private boolean withinTolerance(double v1, double v2) {
    return Math.abs(v1 - v2) < AbstractManipulableDrawable.TOLERANCE;
  }
}
